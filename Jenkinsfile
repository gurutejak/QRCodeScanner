...
node{
    ...
    stage('Build') {
        sh "xcrun xcodebuild  -project '${xcodeProject}' -scheme '${build_scheme}' -destination 'name=iPhone 7' clean build -enableCodeCoverage YES | /usr/local/bin/xcpretty -r junit"
    }
    stage('Archieve'){
